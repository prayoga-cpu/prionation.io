...
function BarPanel({ label, data, xKey }: { label: string; data: { amountEur: number }[]; xKey: string }) {
  return (
    <div className="bg-card border border-line rounded-xl p-5">
      <p className="font-pixel text-[8px] tracking-[0.1em] text-muted uppercase mb-4">{label}</p>
      {data.length === 0 ? (
...
export function IncomeByProject({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-card border border-line rounded-xl p-5">
        <p className="font-pixel text-[8px] tracking-[0.1em] text-muted uppercase mb-4">By month</p>
        {monthData.length === 0 ? (
