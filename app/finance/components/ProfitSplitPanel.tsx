    ...
    <div>
      <div className="grid grid-cols-2 gap-4 mb-3">
        <div className="bg-card border border-line rounded-xl p-5">
          ...
          <p className="font-sans text-2xl font-bold text-white">{formatEur(split.darwinTotal)}</p>
        </div>
        <div className="bg-card border border-line rounded-xl p-5">
          <p className="font-pixel text-[8px] tracking-[0.1em] text-muted uppercase mb-2">
            Evan (20%)
          </p>
          <p className="font-sans text-2xl font-bold text-white">{formatEur(split.evanTotal)}</p>
        </div>
      </div>
      ...
      {split.ownDeals.length > 0 && (
        <p className="font-sans text-[12px] text-muted mb-6">
          ...
          {split.ownDeals.map((d) => `${d.project}, ${formatEur(d.amountEur)}`).join(" · ")} — 100%
          Darwin.{" "}
          ...
          <tbody>
            {split.breakdown.map((t) => (
              <tr key={t.id} className="border-b border-line/50">
                ...
                <td className="font-sans text-[13px] text-soft py-3 pr-4">{formatEur(t.amountEur)}</td>
                <td className="font-sans text-[13px] text-soft py-3 pr-4">
                  {formatEur(t.darwinCutEur)}
                </td>
                <td className="font-sans text-[13px] text-soft py-3 pr-4">
                  {formatEur(t.evanCutEur)}
                </td>
